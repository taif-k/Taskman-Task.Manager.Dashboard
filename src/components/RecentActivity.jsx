...
const ActivityRow = React.memo(({ act, COLUMN_LABELS }) => {
  const getActionText = useCallback((act) => {
    switch (act.type) {
      case "add":
        return (
          <>
            added card <span style={{ textDecoration: "underline" }}>{act.taskName}</span> to board{" "}
      ...
      case "edit":
        return (
          <>
            edited card <span style={{ textDecoration: "underline" }}>{act.taskName}</span> in{" "}
            <span className="fw-semibold text-dark">{COLUMN_LABELS[act.column]}</span>
          </>
        );
      case "delete":
        return (
          <>
            deleted card <span style={{ textDecoration: "underline" }}>{act.taskName}</span> from{" "}
            <span className="fw-semibold text-dark">{COLUMN_LABELS[act.column]}</span>
          </>
        );
      case "move":
        return (
          <>
            moved card <span style={{ textDecoration: "underline" }}>{act.taskName}</span> from{" "}
  ...
  return (
    <Row className="align-items-start mt-3">
      ...
      <Col>
        ...
        {act.image && (
          <Image src={act.image} width={150} height={100} className="img-card" />
        )}
  ...
  return (
    <>
      {recentActivity.map((act, index) => (
        <ActivityRow key={index} act={act} COLUMN_LABELS={COLUMN_LABELS} />
      ))}
    </>
  );
};
